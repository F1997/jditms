...
func (rt *Router) Config(r *gin.Engine) {
	...
	pages := r.Group(pagesPrefix)
	{
		...
		// 采集器配置
		// 内置模版
		pages.GET("/builtin-collect-tpl-cates", rt.builtinCollectCateGets)
		pages.GET("/builtin-collect-tpl/:cate", rt.builtinCollectGet)

		// 添加采集配置
		pages.POST("/collect", rt.auth(), rt.user(), rt.collectAddPost)
		// 查询采集配置列表
		pages.GET("/collects", rt.auth(), rt.user(), rt.collectList)
		// 查询指定采集配置
		pages.GET("/collect/:id", rt.auth(), rt.user(), rt.collectGet)
		// 更新采集配置
		pages.PUT("/collect/:id", rt.auth(), rt.user(), rt.collectUpsert)
		// 是否启用
		pages.PUT("/collect/:id/status", rt.auth(), rt.user(), rt.collectUpdataStatus)
		// 删除采集配置
		pages.DELETE("/collects", rt.auth(), rt.user(), rt.collectDel)

		pages.GET("/busi-group/:id/collects", rt.auth(), rt.user(), rt.perm("/collects"), rt.bgro(), rt.collectGets)
		pages.POST("/busi-group/:id/collects", rt.auth(), rt.user(), rt.perm("/collects/add"), rt.bgrw(), rt.collectAdd)
		// pages.POST("/busi-group/:id/collect/:cid/clone", rt.auth(), rt.user(), rt.perm("/collects/add"), rt.bgrw(), rt.boardClone)

		// 拓扑分析
		// 添加拓扑关系
		pages.POST("/topo", rt.auth(), rt.user(), rt.topoAddPost)
		// 获取拓扑关系列表
		pages.GET("/topos", rt.auth(), rt.user(), rt.topoList)
		// 查看拓扑关系
		pages.GET("/topo/:id", rt.auth(), rt.user(), rt.topoGet)
		// 拓扑图，节点状态
		pages.GET("/topo/:id/status", rt.auth(), rt.user(), rt.topoGetStatus)

		pages.PUT("/topo/:id", rt.auth(), rt.user(), rt.topoUpsert)
		// 删除拓扑图
		pages.DELETE("/topos", rt.auth(), rt.user(), rt.topoDel)

		pages.GET("/busi-group/:id/topos", rt.auth(), rt.user(), rt.perm("/topos"), rt.bgro(), rt.topoGets)
		pages.POST("/busi-group/:id/topos", rt.auth(), rt.user(), rt.perm("/topos/add"), rt.bgrw(), rt.topoAdd)

		// 资源管理
		pages.GET("/resources", rt.auth(), rt.user(), rt.resourceGets)
		pages.GET("/resource/:id", rt.auth(), rt.user(), rt.resourceGet)
		pages.DELETE("/resources", rt.auth(), rt.user(), rt.resourceDel)
		pages.PUT("/resource/:id", rt.auth(), rt.user(), rt.resourceUpsert)
		// 资源列表 业务组
		// pages.GET("/busi-group/:id/resources", rt.auth(), rt.user(), rt.perm("/resources"), rt.resourceGets)
		// pages.GET("/busi-group/:id/resources/:cate", rt.auth(), rt.user(), rt.perm("/resources"), rt.resourceGets)
		pages.POST("/busi-group/:id/resources", rt.auth(), rt.user(), rt.perm("/resources/add"), rt.bgrw(), rt.resourceAdd)
		pages.PUT("/resources/bgid", rt.auth(), rt.user(), rt.resourceUpdateBgid)

		// 资源模版
		pages.GET("/resource-stencils/builtin/list", rt.auth(), rt.user(), rt.resourceStencilBuiltInGets)
		pages.GET("/resource-stencil/:id", rt.auth(), rt.user(), rt.resourceStencilGet)
		pages.PUT("/resource-stencil/:id", rt.auth(), rt.user(), rt.resourceStencilUpsert)
		pages.DELETE("/resource-stencils", rt.auth(), rt.user(), rt.resourceStencilDel)
		pages.POST("/busi-group/:id/resource_stencils", rt.auth(), rt.user(), rt.perm("/resource-stencils/add"), rt.bgrw(), rt.resourceStencilAdd)
		pages.GET("/busi-group/:id/resource-stencils", rt.auth(), rt.user(), rt.perm("/resource-stencils"), rt.resourceStencilGets)
	...
	r.GET("/api/jditms/versions", func(c *gin.Context) {
		v := version.Version
	...
	if rt.HTTP.APIForAgent.Enable {
		heartbeat := r.Group("/v1/jditms")
		{
			...
			heartbeat.GET("/collects", rt.collectGetByAgentHostname)
		}
	}
