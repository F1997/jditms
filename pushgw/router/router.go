...
func (rt *Router) Config(r *gin.Engine) {
	...
	if len(rt.HTTP.APIForAgent.BasicAuth) > 0 {
		...
		r.POST("/v1/jditms/target-update", auth, rt.targetUpdate)
		r.POST("/v1/jditms/edge/heartbeat", auth, rt.heartbeat)

		if len(rt.Ctx.CenterApi.Addrs) > 0 {
			r.POST("/v1/jditms/heartbeat", auth, rt.heartbeat)
		}
	} else {
		// no need basic auth
		...
		r.POST("/v1/jditms/target-update", rt.targetUpdate)
		r.POST("/v1/jditms/edge/heartbeat", rt.heartbeat)

		if len(rt.Ctx.CenterApi.Addrs) > 0 {
			r.POST("/v1/jditms/heartbeat", rt.heartbeat)
		}
	}
}
