...
func TaskTplStatistics(ctx *ctx.Context) (*Statistics, error) {
	if !ctx.IsCenter {
		return poster.GetByUrls[*Statistics](ctx, "/v1/jditms/task-tpl/statistics")
	}
...
func TaskTplGetAll(ctx *ctx.Context) ([]*TaskTpl, error) {
	if !ctx.IsCenter {
		return poster.GetByUrls[[]*TaskTpl](ctx, "/v1/jditms/task-tpls")
	}
...
func TaskTplGetById(ctx *ctx.Context, id int64) (*TaskTpl, error) {
	if !ctx.IsCenter {
		tpl, err := poster.GetByUrls[*TaskTpl](ctx, "/v1/jditms/task-tpl/"+strconv.FormatInt(id, 10))
		return tpl, err
	}
