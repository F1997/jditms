...
func AlertRuleGetsAll(ctx *ctx.Context) ([]*AlertRule, error) {
	if !ctx.IsCenter {
		lst, err := poster.GetByUrls[[]*AlertRule](ctx, "/v1/jditms/alert-rules?disabled=0")
		if err != nil {
			return nil, err
		}
		for i := 0; i < len(lst); i++ {
...
func AlertRuleStatistics(ctx *ctx.Context) (*Statistics, error) {
	if !ctx.IsCenter {
		s, err := poster.GetByUrls[*Statistics](ctx, "/v1/jditms/statistic?name=alert_rule")
		return s, err
	}
...
func GetTargetsOfHostAlertRule(ctx *ctx.Context, engineName string) (map[string]map[int64][]string, error) {
	if !ctx.IsCenter {
		m, err := poster.GetByUrls[map[string]map[int64][]string](ctx, "/v1/jditms/targets-of-alert-rule?engine_name="+engineName)
		return m, err
	}
