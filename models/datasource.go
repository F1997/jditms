...
func GetDatasources(ctx *ctx.Context) ([]Datasource, error) {
	if !ctx.IsCenter {
		lst, err := poster.GetByUrls[[]Datasource](ctx, "/v1/jditms/datasources")
		if err != nil {
			return nil, err
		}
		for i := 0; i < len(lst); i++ {
...
func GetDatasourceIdsByEngineName(ctx *ctx.Context, engineName string) ([]int64, error) {
	if !ctx.IsCenter {
		lst, err := poster.GetByUrls[[]int64](ctx, "/v1/jditms/datasource-ids?name="+engineName)
		return lst, err
	}
...
func DatasourceStatistics(ctx *ctx.Context) (*Statistics, error) {
	if !ctx.IsCenter {
		s, err := poster.GetByUrls[*Statistics](ctx, "/v1/jditms/statistic?name=datasource")
		return s, err
	}
