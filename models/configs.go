...
func ConfigsGet(ctx *ctx.Context, ckey string) (string, error) { //select built-in type configs
	if !ctx.IsCenter {
		if !ctx.IsCenter {
			s, err := poster.GetByUrls[string](ctx, "/v1/jditms/config?key="+ckey)
			return s, err
		}
	}
...
func ConfigsUserVariableStatistics(context *ctx.Context) (*Statistics, error) {
	if !context.IsCenter {
		return poster.GetByUrls[*Statistics](context, "/v1/jditms/statistic?name=user_variable")
	}
...
func ConfigUserVariableGetDecryptMap(context *ctx.Context, privateKey []byte, passWord string) (map[string]string, error) {
	...
	if !context.IsCenter {
		ret, err := poster.GetByUrls[map[string]string](context, "/v1/jditms/user-variable/decrypt")
		if err != nil {
			return nil, err
		}
		return ret, nil
