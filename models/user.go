...
func UserGetAll(ctx *ctx.Context) ([]*User, error) {
	if !ctx.IsCenter {
		lst, err := poster.GetByUrls[[]*User](ctx, "/v1/jditms/users")
		return lst, err
	}
...
func UserStatistics(ctx *ctx.Context) (*Statistics, error) {
	if !ctx.IsCenter {
		s, err := poster.GetByUrls[*Statistics](ctx, "/v1/jditms/statistic?name=user")
		return s, err
	}
...
func (u *User) NopriResourceID(ctx *ctx.Context, resouce_ids []int64) ([]int64, error) {
	if u.IsAdmin() {
		return []int64{}, nil
	}
	...
	var arr []int64
	err = DB(ctx).Model(&Resource{}).Where("group_id in ?", bgids).Pluck("id", &arr).Error
	if err != nil {
		return []int64{}, err
	}

	return slice.SubInt64(resouce_ids, arr), nil
}
