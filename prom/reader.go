...
func (pc *PromClientMap) loadFromDatabase() {
	...
	var err error
	if !pc.ctx.IsCenter {
		datasources, err = poster.GetByUrls[[]*models.Datasource](pc.ctx, "/v1/jditms/datasources?typ="+models.PROMETHEUS)
		if err != nil {
			logger.Errorf("failed to get datasources, error: %v", err)
			return
		}
		lokiDatasource, err := poster.GetByUrls[[]*models.Datasource](pc.ctx, "/v1/jditms/datasources?typ="+models.LOKI)
