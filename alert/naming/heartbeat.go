...
func (n *Naming) ActiveServers(datasourceId int64) ([]string, error) {
	...
	if !n.ctx.IsCenter {
		lst, err := poster.GetByUrls[[]string](n.ctx, "/v1/jditms/servers-active?dsid="+fmt.Sprintf("%d", datasourceId))
		return lst, err
	}
...
func (n *Naming) ActiveServersByEngineName() ([]string, error) {
	if !n.ctx.IsCenter {
		lst, err := poster.GetByUrls[[]string](n.ctx, "/v1/jditms/servers-active?engine_name="+n.heartbeatConfig.EngineName)
		return lst, err
	}
